...
def send_confirm_email(email: str, id: str):
    token = create_access_token(data={"sub": email, "id": id}, expires_delta=timedelta(days=2))
    html_content=f"""
...
@celery.task(serializer='pickle', ignore_result=True)
def send_verfify_code_task(email: str, code: int, name: str, domain: Optional[str] = None):
    subject = get_prefix_email_subject(subject='Verify your Tubekick account - Here\'s your verification code 📬', domain=domain)
    sendinlbue_service._send(
        sender=config['EMAIL_SENDER'],
        emails_to=[email],
        subject=subject,
        params=dict(code=code, name=name),
        template_id=config['USER_VERIFY_EMAIL_TEMPLATE'],
    )

def send_verify_code(email: str, code: int, id: str, name: str, domain: Optional[str] = None) -> str:
        token = create_access_token(
            ...
            expires_delta = timedelta(hours=config['EXPIRES_VERIFY_TOKEN_DELTA'])
        )
...
@celery.task(serializer='pickle', ignore_result=True)
def send_reset_password_task(email: str, id: str, name: str, domain: Optional[str] = config['USER_CONFIRM_EMAIL_URL']):
    subject = get_prefix_email_subject(subject='Reset your Tubekick password - Your password reset link is here 🔒', domain=domain)
    token = create_access_token(
        data={"sub": email, "id": id, 'grant_type': AuthGrantType.RESET_PASSWORD.value},
        expires_delta=timedelta(hours=config['EXPIRES_RESET_TOKEN_IN_HOURS'])
    )
...
def send_user_welcome_email_task(email: str, name: str, domain: Optional[str] = None):
    subject = get_prefix_email_subject(subject='Welcome to the Tubekick Reseller Platform!', domain=domain)
    sendinlbue_service._send(
        sender=config['MAIL_SENDER'],
        emails_to=[email],
        subject=subject,
        template_id=config['USER_WELCOME_EMAIL_TEMPLATE'],
...
def send_welcome_email(email: str, name: str, domain: Optional[str] = None):
    if config['ENVIRONMENT'] and config['ENVIRONMENT'] != 'local':
        celery_delay(
            ...
            name=name,
            domain=domain,
        )(task_id='send_user_welcome_email_to_{}'.format(email))
# endregion
...
def send_user_top_up_confirm_email_task(
    ...
    subject = get_prefix_email_subject(subject='We have topped up your Tubekick Wallet!', domain=domain)
    sendinlbue_service._send(
        sender=config['EMAIL_SENDER'],
        emails_to=[email],
        subject=subject,
